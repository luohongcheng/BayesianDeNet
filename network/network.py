...
class Network(object):
    ...
    def make_var(self, name, shape, regularizer=None):
        '''Creates a new TensorFlow variable.'''
        # weight decay
        return tf.get_variable(name, shape, dtype='float32', trainable=self.trainable, regularizer=regularizer)

    def validate_padding(self, padding):
        '''Verifies that the padding is one of the supported ones.'''
        assert padding in ('SAME', 'VALID')

    @layer  # bilinear_resize layer
    def upsample_bilinear(self, input_data, input_size, name):
        return tf.image.resize_bilinear(input_data, size=input_size, name=name)
    ...
    def conv(self,
        ...
        with tf.variable_scope(name) as scope:
            ...
            kernel = self.make_var('weights', shape=[k_h, k_w, c_i // group, c_o],
                                   regularizer=tf.contrib.layers.l2_regularizer(tf.constant(0.0005, dtype=tf.float32)))
            ...
            if biased:
                # weight decay
                biases = self.make_var('biases', [c_o], regularizer=tf.contrib.layers.l2_regularizer(
    ...
    def batch_normalization(self, input_data, name, scale_offset=True, relu=False):
        ...
        with tf.variable_scope(name) as scope:
            ...
            pop_mean = tf.get_variable("mean", shape, initializer=tf.constant_initializer(0.0), trainable=True)
            pop_var = tf.get_variable("variance", shape, initializer=tf.constant_initializer(1.0), trainable=True)
            epsilon = 1e-4
            decay = 0.999
            if scale_offset:
